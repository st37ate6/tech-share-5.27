...
// 创建大型项目模拟
function createLargeProject(fileCount) {
...
// 单线程处理
function processProjectSingleThread(project) {
...
// 模拟并发处理（JavaScript 单线程，使用 Promise 模拟）
...
async function processProjectHighConcurrency(project) {
  const start = process.hrtime.bigint();
  const total = project.files.length;
  let processed = 0;
...
// 模拟文件处理
function processFile(file, globalSymbols) {
  // 模拟 AST 遍历
...
// 带依赖关系的文件处理
function processFileWithDependencies(file, globalSymbols, dependencies) {
  // 基本处理
...
// 复杂的 AST 遍历
function visitNodeComplex(node) {
  if (!node) {
    return 0;
  }
...
// 全局符号解析
function resolveSymbolWithGlobal(name, globalSymbols) {
  return globalSymbols.getSymbol(name);
}

// 类型检查
function performTypeCheck(globalSymbols) {
  // 简化类型推导
  let count = 0;
  for (const [, typeInfo] of globalSymbols.types) {
...
// 增量编译检查
function checkIncrementalChanges(file, globalSymbols) {
  // 简化增量编译逻辑
  for (let i = 0; i < 10; i++) {
...
// 主函数
async function main() {
  console.log('=== 大规模 JavaScript 测试 ===\n');

  // 调整测试规模，使其更合理
  const fileCounts = [50, 200, 500];
